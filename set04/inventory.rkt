...
;; Where: two different books in the inventory
;; don't have the same isbn
...
;; Where: two different lineitems don't have the same isbn
...
;; =21000
;; STRATEGY: HOFC + structural decompostion on b : Book
(define (inventory-potential-profit lob)
  (foldr
   ;; Book Integer ->  Integer
   ;; GIVEN: a book and an the price of the rest of books
...
(define (inventory-total-volume lob)
  (foldr
   ;; Book Real ->  Real
   ;; GIVEN: a book and a real number
   ;; RETURNS: the total volumen needed by one kind of book
   (lambda (b total-for-rest)
     (+ (* (book-volume b) (book-on-hand b)) 
        total-for-rest))
   ZERO
   lob))
...
;; STRATEGY: HOFC + structural decomposition on b : Book
...
(define (price-for-order in ord)
  (foldr
   ...
   ;; GIVEN: the days needed for the book and the rest of the list
   ;; RETURNS: the total of them
   (lambda (li total-of-rest)
     (+ (mi->integer (price-for-line-item in li))
        total-of-rest))
   ZERO
   ord))
...
(define (price-for-line-item in li)
  (price-for-line-item-helper
   (filter 
    ;; Book -> Boolean
    ;; GIVEN: a book
    ;; RETURNS: wehter a book contains the same isbn with the line item
    (lambda (b) (same-isbn? b li)) in) li))
 
;; price-for-line-item-helper : Inventory LineItem -> MaybeInteger
;; GIVEN: an inventory and a line item
;; RETURNS: the price for that line item (the quantity times the unit
;; price for that item).  Returns false if that isbn does not exist in
;; the inventory.
;; EXAMPLE
;; (book-after-line-item-helper 
;;  (make-book 12345 "computer" "Wand" "ABC" 1200 900 30 r1 12) 
;;  (make-line-item 12345 15))
;; =15
;; STRATEGY: structural decompostion on in : Inventory
(define (price-for-line-item-helper in li)
  (cond
    [(empty? in) false]
    [else (price-each-book (first in) li)]))
...
(define (inventory-after-order in ord)
  (map
   ;; Book -> Book
   ;; GIVEN: a book
   ;; RETURNS: a book after an order has been filled
   (lambda (b)
     (book-after-order b ord))
   in))
...
(define (book-after-order b ord)
  (book-after-order-helper
   (filter 
    ;; Book -> Boolean
    ;; GIVEN: a book
    ;; RETURNS: wehter a book contains the same isbn with the line item
    (lambda (li) (same-isbn? b li)) ord) b))

;; book-after-order-helper : Book Order -> Book.
;; GIVEN: a book, an order
;; WHERE: the order is fillable now
;; RETURNS: the book after the order has been filled.
;; EXAMPLE
...
(define (book-after-order-helper ord b)
  (cond
    [(empty? ord) b]
    [else (book-after-lineitem b (first ord))]))
...
(define (fillable-now? ord in)
  (andmap
   ;; Book -> Boolean
   ;; GIVEN: a book
   ;; RETURNS: whether a book in invnetory is fillable now
   (lambda (b)
     (lineitem-fillable-now? b in))
   ord))
...
;; = true
;; STRATEGY: structural decompostion on li : LineItem
(define (book-lineitem-fillable? li b)
  (and (same-isbn? b li) 
...
(define (days-til-fillable ord in)
  (foldr
   ;; LineItem MaybeInt -> MaybeInt
   ;; GIVEN: a line item and a maybeinteger of the rest of the order
   (lambda (li rest)
     (days-needed (days-til-fillable-lineitem li in) rest))
   ZERO
   ord))
...
(define (days-til-fillable-lineitem li in)
  (days-til-fillable-lineitem-helper
   (filter 
    ;; Book -> Boolean
    ;; GIVEN: a book
    ;; RETURNS: wehter a book contains the same isbn with the line item
...
;; days-til-fillable-lineitem-helper : Inventory LineItem -> MaybeInteger
;; GIVEN: a lineitem and an inventory
;; RETURNS: the number of days until the lineitem is fillable
;; EXAMPLE
;; (days-til-fillable-line-item-inventory 
...
(define (days-til-fillable-lineitem-helper in li)
  (cond
    [(empty? in) false]
    [else (days-til-fillable-li-book (first in) li)]))
...
;; STRATEGY: HOFC + structural decomposition on b : Book
(define (inventory-after-deliveries in)
  (map
   ;; book-after-deliveries : Book -> Book
   ;; GIVEN: today's book
   ;; RETURNS: a book representing tomorrow's status
   ...
   (lambda (b)
     (make-book (book-isbn b)
                ...
                (book-unit-price b) 
                (book-unit-cost b) 
                (on-hand-tomorrow (book-on-hand b) (book-reorder-status b))
                (rs-tomorrow (book-reorder-status b))
                (book-volume b)))
  in))

;; on-hand-tomorrow : Integer ReorderStatus -> Integer
;; GIVEN: on-hand number and the reorder stauts
;; RETURNS: the on-hand number tomorrow
;; EXAMPLE:
;; (on-hand-tomorrow 10 r3) => 15
;; STRATEGY: structural decomposition on rs : ReorderStatus
(define (on-hand-tomorrow on-hand rs)
  (if (= ONE (reorder-status-days rs)) 
      (+  on-hand (reorder-status-copies rs))
      on-hand))

;; rs-tomorrow : ReorderStatus -> ReorderStatus
;; GIVEN: the reorder stauts
;; RETURNS: the reorder stauts tomorrow
;; EXAMPLE:
;; (rs-tomorrow r3) => r1
;; STRATEGY: structural decomposition on rs : ReorderStatus
(define (rs-tomorrow rs)
  (cond
    [(not (reorder-status-present? rs)) rs]
    [else (rs-tomorrow-helper (reorder-status-days rs) 
                              (reorder-status-copies rs))]))

;; rs-tomorrow-helper : Integer Integer -> ReorderStatus
;; GIVEN: the reorder stauts days and copies
;; RETURNS: the reorder stauts tomorrow
;; EXAMPLE: see test
;; (rs-tomorrow r3) => r1
;; STRATEGY: cases on days
(define (rs-tomorrow-helper days copies)
  (cond
    [(= ONE days) (make-empty-reorder ZERO)]
    [else (make-reorder (- days ONE) copies)]))
...
(define r3 (make-reorder-status true 1 15))
(define r4 (make-reorder-status true 1 30))
...
(define b5 
  (make-book 12348 "computer" "Marshall" "America" 1600 1200 30 r3 13))
(define b6 
  (make-book 12346 "computer" "Stephen" "ABC" 1300 800 30 r2 13))
...
(begin-for-test
  ...
  ; Test inventory-after-deliveries
  (check-equal? (inventory-after-deliveries i5)
                (list
                 (make-book 12345 
                            "computer" 
                            "Wand" 
                 ...
                 (make-book 12348 
                            ...
                            13)
                 b6)
                "Test inventory-after-deliveries, it will change rs and on-hand for
