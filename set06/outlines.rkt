...
(define INT-SEXP-0 0)
(define INT-SEXP-7 7)
(define LOS-SEXP-1 (cons STRING-SEXP empty))
(define LOS-SEXP-2 (cons STRING-SEXP (cons INT-SEXP-0 empty)))
(define LOS-SEXP-3 (cons INT-SEXP-0 (cons INT-SEXP-7 empty)))
...
; -- empty
; -- (cons NestedSexp NestedRep)

; A NestedSexp is a (cons String NestedRep)
...
(begin-for-test 
  ...
  (check-equal? (nested-rep? LOS-SEXP-1) false
                "ListOfSexp with String is not nested")
  (check-equal? (nested-rep? LOS-SEXP-2) false
                "ListOfSexp with String and NonNegInt is not nested")
  (check-equal? (nested-rep? LOS-SEXP-3) false
                "ListOfSexp with NonNegInt and NonNegInt is not nested"))
...
(define (nested-rep? s)
  (cond [(string? s) false]
        [(integer? s) false]
        [(list? s) (losexp-is-nested-rep? s)]))


;; losexp-is-nested-rep? : ListOfSexp -> Boolean
;; Returns true iff the given ListOfSexp is a valid NestedRep
...
;; Strategy: Structural Decomposition on l : ListOfSexp

(define (losexp-is-nested-rep? l)
  (cond [(empty? l) true]
        [else (and (sexp-is-nested-sexp? (first l))
                   (losexp-is-nested-rep? (rest l)))]))

;; sexp-is-nested-sexp? : Sexp -> Boolean
;; Returns true iff the given Sexp is a NestedSexp

;; Strategy: Structural Decomposition on s : Sexp
(define (sexp-is-nested-sexp? s)
  (cond [(string? s) false]
        [(integer? s) false]
        [(list? s) (sexp-is-nested-sexp?-helper s)]))

;; sexp-is-nested-sexp?-helper : ListOfSexp -> Boolean
;; Returns true iff the given ListOfSexp is a NestedSexp

;; Strategy : Structural Decomposition on los : ListOfSexp
(define (sexp-is-nested-sexp?-helper los)
  (cond [(empty? los) false]
        [else (and (string? (first los))
                   (losexp-is-nested-rep? (rest los)))]))
